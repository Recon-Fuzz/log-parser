...
function processMedusa(line, jobStats) {
    ...
    if (line.includes("fuzz: elapsed:")) {
        jobStats.duration = (0, utils_1.captureFuzzingDuration)(line.replace("fuzz: elapsed:", "")) ?? ""; // TODO 0XSI - fix this
        ...
        if (brokenProp !== "" || currentBrokenPropertyMedusa !== "") {
            ...
            const existingProperty = jobStats.brokenProperties.find((el) => el.brokenProperty === currentBrokenPropertyMedusa);
            if (!existingProperty) {
...
function getPropertyAndSequenceString(logs, vmData) {
    ...
    const bodies = splitted.map((entry) => vmData
...
function getFunctionCallsWithVM(logs, vmData) {
    const pattern = /(\w+)\(([^)]*)\)\s+\(block=\d*, time=\d*, gas=\d*, gasprice=\d*, value=\d*, sender=\d*x\d*/gm;
...
function medusaLogsToFunctions(logs, identifier, vmData) {
    ...
    const filtered = testsToBuild.filter((entry) => Array.isArray(entry.sequence));
    const unableToProcess = testsToBuild.filter((entry) => !Array.isArray(entry.sequence));
