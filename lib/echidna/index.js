...
export function processEchidna(line, jobStats) {
    ...
    if (line.includes("[status] tests:")) {
        ...
        if (line === "" && echidnaTraceLogger || line.includes("Saved reproducer")) {
            ...
            const existingProperty = jobStats.brokenProperties.find((el) => el.brokenProperty === currentBrokenPropertyEchidna);
            if (existingProperty &&
...
function cleanUpBrokenPropertyName(brokenProp) {
...
export function echidnaLogsToFunctions(input, prefix, brokenProp, vmData) {
    ...
    return callSequenceMatches
        ...
        let updated = test
            .trim()
            .replace(/\)/g, ");")
            .replace("Call sequence", `function ${brokenProp ? `test_${brokenProp}_${prefix}` : `test_prefix_${i}_${prefix}`}() public {`)
            // Fixing shitty regex
            .replace("{,", "{")
            .replace("{:", "{")
